...
def calculate_postage(city: str, country: str, weight: float) -> float:
	...
	distance = _calculate_distance(originLatitude, originLongitude, targetLatitude, targetLongitude)

	base_rate = 25.0  # Base rate for domestic postage
	weight_rate = 4  # Rate per kilogram
...
def _parse_cli_args(argv: list[str]) -> tuple[str, str, float]:
	"""
	Parse command-line arguments.

	Supported forms:
	- key=value style: city=Sydney country=Australia weight=2.5
	 — Values that contain spaces should be quoted by the shell, e.g. city="Hong Kong"
	- positional style: <city> <country> <weight>
	...
	country = parsed.get('country') or (positionals[1] if len(positionals) > 1 else None)
	weight_str = parsed.get('weight') or (positionals[2] if len(positionals) > 2 else None)
	...
	return city, country, weight


def main():
	try:
		city, country, weight = _parse_cli_args(sys.argv)
	except Exception as e:
		print(f"Error parsing arguments: {e}")
		print('Usage examples:')
		print('  python domestic_postage.py city="Hong Kong" country="Hong Kong" weight=2.5')
		return
